...
const Draggable = ({
  ...
  const mouseMoveHandler = event => {
    ...
    if
      (event.clientY <= 0 || event.clientX <= 0 || (event.clientX >= window.innerWidth || event.clientY >= window.innerHeight)) {
        setIsAttached(false)
    }
  }
  ...
  return (
          <div
            style={{
              position: 'absolute',
              cursor: 'pointer',
              left: isAttached ? mousePosition.x  - (clickedMousePosition.x - elementPosition.x) : elementPosition.x,
              top: isAttached ? mousePosition.y  - (clickedMousePosition.y - elementPosition.y) : elementPosition.y,
            }}
